    ...
    async function persistStateNow() {
      ...
      try {
        ...
        if (response.status === 409) {
          // Do NOT discard local edits here. Another session/tab saved a newer
          // revision, but the data the user just typed in *this* tab (a
          // movement, an initial-stock number, etc.) is still only in memory
          // and hasn't reached the server yet. Overwriting state with the
          // server's older-than-our-edit snapshot is what made entered data
          // "disappear". Instead, adopt the new revision number and resend
          // our local state as-is, so the user's own work always wins.
          ...
          alert("Ma'lumotlarni saqlashda ketma-ket to'qnashuvlar yuz berdi (sahifa bir nechta oynada ochiq bo'lishi mumkin). Boshqa oynalarni yoping va qaytadan saqlab ko'ring.");
          return;
        }
    ...
    function bindEvents() {
      ...
      document.addEventListener("input", (e) => {
        ...
        if (!field) {
          const initialStockField = e.target.closest("[data-initial-stock-id]");
          if (initialStockField) {
            // Commit the value into state as soon as it's typed, instead of
            // only reading it from the DOM when the form is submitted. The
            // initial-stock table is paginated (30 rows/page) and only the
            // current page's inputs exist in the DOM, so waiting for submit
            // meant switching pages (or search filter, or a re-render caused
            // by a save conflict) silently reset unsaved rows back to 0.
            const productId = initialStockField.dataset.initialStockId;
            const raw = String(initialStockField.value || "").trim();
            const parsed = raw === "" ? 0 : Number(raw);
            if (Number.isFinite(parsed) && parsed >= 0 && Number.isInteger(parsed)) {
              state.initialStock[productId] = parsed;
              const row = initialStockField.closest("tr");
              const qtyCell = row ? row.children[2] : null;
              const product = getProduct(productId);
              if (qtyCell && product) qtyCell.textContent = qtyToText(product, parsed);
              const totalPieces = state.products.reduce((acc, p) => acc + Math.max(0, toInt(state.initialStock[p.id]) || 0), 0);
              if (el.initialStockTotal) {
                el.initialStockTotal.textContent = `Jami boshlang'ich ostatka: ${totalPieces.toLocaleString("uz-UZ")} dona`;
              }
            }
            return;
